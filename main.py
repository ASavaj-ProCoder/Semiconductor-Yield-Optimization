...
# 4.FEATURE SELECTION(Fixes high-dimensional sensor noise)
#Select the top 40 most statistically significant features as requested by the baseline constraints
...
#Convert back to structured DataFrames for model training
X_train = pd.DataFrame(X_train_selected)
X_test = pd.DataFrame(X_test_selected)
print(f"Post-Selection Training Shape: {X_train.shape} (Successfully targeted top 40 critical features)")

# 5.TUNED MODEL TRAINING
...
# 8.METRIC PLOTTING: PRECISION-RECALL CURVE
precision, recall, thresholds = precision_recall_curve(y_test, probabilities)

plt.figure(figsize=(7, 5))
plt.plot(recall, precision, color='#1f77b4', lw=2.5, label='Random Forest Production Curve')

#Find closest precision/recall values for plotting the operating dot
operating_idx = np.argmin(np.abs(thresholds - custom_threshold))
plt.scatter(recall[operating_idx], precision[operating_idx], color='red', marker='o', s=120, zorder=5, 
